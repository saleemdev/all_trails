#!/usr/bin/env python3
"""
Build script for ALL_TRAILS frontend

This script is called by Frappe's build system to:
- Run Vite build to generate hashed assets
- Read Vite manifest.json to get correct asset filenames
- Update HTML template with fallback hashes for production
- Ensure CSRF token is properly injected
"""
...
def build():
    ...
    try:
        print("=" * 60)
        print("Building ALL_TRAILS frontend...")
        print("=" * 60)
...
def extract_asset_hashes():
    ...
    """
    try:
        # Path to Vite manifest - relative to frontend directory
        manifest_path = "../all_trails/public/frontend/.vite/manifest.json"
...
def update_html_template(js_file, css_file):
    ...
    """
    try:
        dest_html = "../all_trails/www/all-trails.html"
